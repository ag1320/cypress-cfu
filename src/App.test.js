...
test('multiple recipe names from state appears in an unordered list', async () => {
  let {instructionsInput, nameInput, submitButton} = setup();
  ...
  let {instructionsInput1, nameInput1, submitButton1} = setup();
  const recipeName2 = "Mashed potatoes"
  const recipeInstructions2 = "boilem mashem stickem in a stew"

  await userEvent.type(instructionsInput1, recipeInstructions2)
  await userEvent.type(nameInput1, recipeName2)
